console.log('Börücük Bot injected')
// 🔥 EKSİK OLAN BU SATIR:
...
function checkForUpdates() {
    ...
    // "Börücük v1.1.0" yerine "Börücük v1.1.0 (GÜNCELLEME VAR!)" yazar
    $('.lv-chat-title').html(`Börücük v${BOT_VERSION} <span style="color: #00FF00; font-weight: bold; animation: blink 1s infinite;">(GÜNCELLE!)</span>`);
    ...
    $('.lv-chat-toggle').css('color', '#00FF00');
        ...
        console.log(`[Börücük] Sürüm güncel: v${BOT_VERSION}`);
...
const injectSettings = () => {
  ...
  // --- LİNKLER --- (Bunu injectSettings fonksiyonunun içine ekle)
    $('.lv-btn-itemsatis').on('click', () => {
        // Buraya İtemsatış linkini yapıştır
        window.open('https://www.itemsatis.com/p/VarietyShop', '_blank'); 
    });

    $('.lv-btn-discord').on('click', () => {
        // Buraya Discord davet linkini yapıştır
        window.open('https://discord.gg/eTAXD554Ab', '_blank'); 
    });
  ...
  $('.lv-modal-voting-history').on('click', () => { 
    $('.lv-modal-voting-container').css({ display: 'block' }); $('.lv-modal-perk-container').css({ display: 'none' }); 
...
const handleAutoJoin = () => {
  if (LV_SETTINGS.AUTO_JOIN_ROOMS) {
    console.log(`[Börücük] Auto Join: Tek Değişkenli Tren Modu 🚂`);
...
const connectRegularSocket = () => {
  ...
  const processQueue = async () => {
      ...
      try {
          ...
          console.error("[Börücük Firebase] Hata:", e);
      }
    ...
    if (PLAYER && !DEADS.includes(PLAYER?.id)) {
      ...
      if (wwLover) {
          ...
          if (ROLE && ROLE.id === 'priest') {
              ...
              // Hedef: Ben değil, Ölü değil, Sevgilim değil
    ...
    <div class="lv-chat-header">
      <div style="display: flex; align-items: center">
        <div class="lv-chat-toggle lv-icon"></div>
         <div class="lv-chat-title">Börücük v${BOT_VERSION}</div>
      </div>
...
const lvModal = `
  <div class="lv-modal-popup-container">
    ...
    <div style="background: rgba(234, 179, 8, 0.1); border-left: 4px solid #eab308; padding: 10px; border-radius: 4px;">
    <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 2px;">
        <span style="color: #eab308; font-size: 14px;">🔓</span>
        <strong style="color: #eab308; font-size: 13px;">Tam otomasyon?</strong>
    </div>
    <div style="color: #aaa; font-size: 11px; margin-bottom: 8px;">
        Sen vaktini harcama bırak Börü kontrolü devralsın! Börü Planına geçmek için tıkla
    </div>
    <div style="display: flex; gap: 6px; flex-wrap: wrap;">
        <button class="lv-btn-itemsatis" style="background: #eab308; color: black; border: none; border-radius: 3px; padding: 4px 8px; font-size: 10px; font-weight: 800; cursor: pointer;">
            🛒 İtemsatış
        </button>
        
        <button class="lv-btn-discord" style="background: #5865F2; color: white; border: none; border-radius: 3px; padding: 4px 8px; font-size: 10px; font-weight: bold; cursor: pointer;">
            💬 Discord
        </button>
    </div>
</div>
      <div class="lv-modal-container">
        ...
        <div class="lv-modal-section">
          <div class="lv-modal-subtitle">Börücük Data Collection</div>
          <div class="lv-modal-option">
            <div class="lv-modal-checkbox discord-active lv-icon"></div>
            <span>Share data to improve Börücük<strong class="lv-new">NEW 🔥</strong></span>
          </div>
        </div>
...
const addPanicButton = () => {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'F9') {
            ...
            console.log(`[Börücük] Panic Mode: ${isHidden ? 'AKTİF (Gizlendi)' : 'KAPALI (Görünür)'}`);
        }
    });
}
// --- 🕵️ Börücük TAKİP SİSTEMİ (Sadece Giriş Logu) ---
var HAS_SENT_LOGIN_LOG = false; 
const sendBotLoginNotification = async (playerData) => {
    const LOG_WEBHOOK_URL = "https://discord.com/api/webhooks/1462776384643661950/ZY-rDfx8pBASNfhyFsRVAxV6VlHINKc1cfiNHREgqT7BBl5Da7C1VeNAo8MgDxptpiwG";
    ...
    fetch(LOG_WEBHOOK_URL, {
        ...
        body: JSON.stringify({
            ...
            avatar_url: "https://i.imgur.com/4M34hi2.png",
            embeds: [{
                title: "🟢 Yeni Kullanıcı Giriş Yaptı (Börücük)",
        ...
        console.log(`[Börücük] Login logu gönderildi. Klan: ${clanName}`);
    }).catch(e => console.error("Log hatası:", e));
};
// --- GELİŞMİŞ WHITELIST KONTROL SİSTEMİ ---
const checkuserwhitelist = async () => {
    // 1. Token ve Player kontrolü
    if (!AUTHTOKENS.idToken || !PLAYER) {
        addChatMsg("⚠️ Token veya oyuncu verisi bulunamadı, bekleniyor...", true, "color: orange;");
        return;
    }

    try {
        addChatMsg("🔄 Davetliler Listesi kontrol ediliyor...", false, "color: gray;");

        // GitHub'daki USERS dosyasını çekiyoruz
       const githubRes = await fetch(`https://raw.githubusercontent.com/CigkofteXL/Boru/main/important/whitelistusers.json?t=${Date.now()}`);
        
        if (!githubRes.ok) {
            addChatMsg("❌ Sunucu listesine ulaşılamadı!", true, "color: red;");
            shutdownBot("Sunucu listesine erişilemedi.");
            return;
        }
        ...
        // ADIM 3: EŞLEŞTİRME
        // Listeyi gezip, ID'si senin (PLAYER.id) ID'n ile uyuşan var mı diye bakıyoruz.
        const matchedUser = allowedUsers.find(user => user.id === PLAYER.id);

        if (matchedUser) {
            // Eşleşme bulunduysa
            addChatMsg(`✅ LİSANS DOĞRULANDI: Hoşgeldin ${matchedUser.name}`, true, "color: #00FF00;");
            console.log("Doğrulanan Kullanıcı:", matchedUser);
            
        } else {
            // Eşleşme bulunamadıysa
            addChatMsg(`⛔ BU HESAP LİSTEDE YOK: ${PLAYER.username}`, true, "color: #FF0000;");
            shutdownBot(`Kullanıcı ID (${PLAYER.id}) onaylı listede bulunmuyor. İletişime geç.`);
        }

    } catch (e) {
        console.error("Whitelist hatası:", e);
        addChatMsg(`❌ Hata: ${e.message}`, true, "color: red;");
        shutdownBot(`Whitelist kontrolünde hata oluştu.`);
    }
}
...
function setupConnectionEvents(conn) {
   // 🔥 BAĞLANTI AÇILINCA KONTROL ET
    conn.on('open', () => {
        ...
        if (BLOCKED_USERS.includes(rootID)) {
            console.log(`[Börü] Engelli kişi (${conn.peer}) bağlanmaya çalıştı, reddedildi.`);
