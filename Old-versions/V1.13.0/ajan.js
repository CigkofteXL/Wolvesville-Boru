  ...
  EventTarget.prototype.addEventListener = function (type, listener, options) {
    ...
    if (['visibilitychange', 'blur', 'pagehide'].includes(type)) {
      const safeListener = function (e) {
        if (!e) return // Event objesi yoksa hiç bulaşma

        // 1. Durdurma (Propagation) yetkilerini kullan (Bunlar genelde hep çalışır)
        ...
        // 2. İptal etme (Default eylemi bozma) - ASIL HATA BURADAN ÇIKIYORDU
        // Sadece cancelable === true ise ve passive değilse preventDefault yap!
        if (e.cancelable && typeof e.preventDefault === 'function') {
          try {
            e.preventDefault()
          } catch (err) {
            // Passive listener hatası vs olursa sessizce yut, log bile atma
          }
        }
      }
      ...
      if (type === 'message') {
        const hookedListener = function (event) {
          ...
          if (typeof data === 'string') {
            if (
              data.includes('error-game-connection-lost') ||
              data === '41' ||
              data.startsWith('41') ||
              data.includes('error-multi-device')
            ) {
