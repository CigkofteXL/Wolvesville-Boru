...
// 🔥🔥 1. CLOUDFLARE KORUMASI: Eğer bu ekrandaysak çalışmayı DURDUR 🔥🔥
if (document.title === "Just a moment..." || document.getElementById("challenge-form") || document.title.includes("Cloudflare")) {
    ...
    // Scriptin geri kalanını okumasını engellemek için throw atıyoruz
...
const addHotkeys = () => {
    ...
    window.addEventListener('keydown', (e) => {
       ...
       // 🔥 BÖRÜ ENTER KORUMASI VE DİREKT ATEŞLEME 🔥
        ...
        if (e.ctrlKey && e.altKey) {
            ...
            // COMMAND PALETTE (Ctrl + Alt + G veya senin belirlediğin tuş)
            if (e.code === 'Key' + (LV_SETTINGS.SHORTCUTS?.COMMAND || 'G')) {
                e.preventDefault(); e.stopPropagation();
                const cmdPalette = $('#boru-cmd-palette');
                
                if (cmdPalette.is(':visible')) {
                    cmdPalette.hide();
                } else {
                    cmdPalette.css('display', 'flex');
                    $('#boru-cmd-input').val('').focus();
                }
            }
        }
        ...
        // --- KLAVYEDEN "/" TUŞUNA BASINCA DİREKT AÇILMA ---
        if (e.key === '/') {
            e.preventDefault(); // Tarayıcının kendi aramasını engelle
            const cmdPalette = $('#boru-cmd-palette');
            cmdPalette.css('display', 'flex');
            $('#boru-cmd-input').val('/').focus();
            setTimeout(() => $('#boru-cmd-input').trigger('input'), 10); // Listeyi anında tetikle
            return;
        }
...
let DAY_VOTING = [];
let GAME_VOTING = "";
// İçine yazdığın ms kadar banko bekler, üstüne 0-600ms rastgele ekler
const gecikmelan = (bankoBekleme = 0) => bankoBekleme + Math.floor(Math.random() * 600);
// 🔥 6 haneli rastgele pID (Public/Packet ID) üretir
const generatePid = () => {
...
if (isMobile) {
  ...
  getAuthtokens()
  loadSettings()
  setTimeout(baslatKarargahTelsizi, 5000) // Telsizi başlatmayı biraz geciktiriyoruz
  // Lisans kontrolünden hemen önce çıkarsa çok daha inandırıcı olur.
  triggerAprilFoolsTroll();
  injectChat()
  injectSettings()
  injectStyles()
  checkForUpdates()
...
const injectSettings = () => {
  ...
  $('body').append(boruCmdPalette)
  $('body').append(boruBugModal);
  

  // --- KOMUT PALETİ AKILLI DİNLEYİCİ (INTELLISENSE + OK TUŞLARI) ---
  let currentSuggestionIndex = -1; // O an seçili olan satırın numarası

  // 1. Önerileri Ekrana Çizme
  $('#boru-cmd-input').on('input', function() {
      const val = $(this).val();
      const suggBox = $('#boru-cmd-suggestions');
      suggBox.empty();
      currentSuggestionIndex = -1; // Yazı değişince seçimi sıfırla
      ...
      // Boşluklara göre ayır, boşlukları da say! (Örn: "/bug " -> ["/bug", ""])
      const parts = val.split(' ');
      const cmd = parts[0].toLowerCase();
      let html = "";

      // --- AŞAMA 1: Sadece komut yazıyorsa ---
      if (parts.length === 1) {
          if ('/msg'.startsWith(cmd)) {
              html += `<div class="cmd-sugg-item" data-suggestion="/msg " style="padding: 10px; margin: 2px 0; background: transparent; border-left: 3px solid transparent; border-radius: 4px; color: #aaa; font-size: 13px; transition: background 0.1s; cursor: pointer;">
                  <span style="color: #fb2e00; font-weight: bold;">/msg</span> <span style="color: #ffc300;">[Kullanıcı]</span> <span style="color: #fff;">[Mesaj]</span>
              </div>`;
          }
          if ('/bug'.startsWith(cmd)) {
              html += `<div class="cmd-sugg-item" data-suggestion="/bug " style="padding: 10px; margin: 2px 0; background: transparent; border-left: 3px solid transparent; border-radius: 4px; color: #aaa; font-size: 13px; transition: background 0.1s; cursor: pointer;">
                  <span style="color: #fb2e00; font-weight: bold;">/bug</span> <span style="color: #fff;">[Karşılaştığınız Hata]</span>
              </div>`;
          }
          if ('/oneri'.startsWith(cmd) || '/öneri'.startsWith(cmd)) {
              html += `<div class="cmd-sugg-item" data-suggestion="/oneri " style="padding: 10px; margin: 2px 0; background: transparent; border-left: 3px solid transparent; border-radius: 4px; color: #aaa; font-size: 13px; transition: background 0.1s; cursor: pointer;">
                  <span style="color: #fb2e00; font-weight: bold;">/oneri</span> <span style="color: #fff;">[Fikriniz]</span>
              </div>`;
          }
           if ('/bigbio'.startsWith(cmd)) {
              html += `<div class="cmd-sugg-item" data-suggestion="/bigbio " style="padding: 10px; margin: 2px 0; background: transparent; border-left: 3px solid transparent; border-radius: 4px; color: #aaa; font-size: 13px; transition: background 0.1s; cursor: pointer;">
                  <span style="color: #fb2e00; font-weight: bold;">/bigbio</span> <span style="color: #fff;">[Büyük Biyografi]</span>
              </div>`;
          }
      } 
      // --- AŞAMA 2 ve 3: Komuttan sonra boşluk bırakılmışsa ---
      else if (parts.length >= 2) {
          
          // /msg komutunun devamı
          if (cmd === '/msg' || cmd === '/sendmessage') {
              if (parts.length === 2) { // İsim arama evresi
                  const searchName = parts[1].toLowerCase();
                  let friends = [];
                  $('.vs-user-item').not('.offline').each(function() {
                      const name = $(this).attr('data-username');
                      if (name && name.toLowerCase().startsWith(searchName)) {
                          friends.push(name);
                      }
                  });
                  ...
                  if (friends.length > 0) {
                      friends.forEach((f) => {
                          html += `<div class="cmd-sugg-item" data-suggestion="${cmd} ${f} " style="padding: 10px; margin: 2px 0; background: transparent; border-left: 3px solid transparent; border-radius: 4px; color: #aaa; font-size: 13px; transition: background 0.1s; cursor: pointer;">
                              <span style="color: #fb2e00; font-weight: bold;">${cmd}</span> <span style="color: #ffc300;">${f}</span>
                          </div>`;
                      });
                  } else {
                      html += `<div style="padding: 10px; color: #555; font-size: 12px; font-style: italic;">Listede eşleşen biri yok...</div>`;
                  }
              } else { // Mesaj yazma evresi (İsim seçilmiş)
                  html += `<div style="padding: 10px; color: #aaa; font-size: 13px; font-style: italic;">
                      <span style="color: #fb2e00; font-weight: bold;">${cmd}</span> <span style="color: #ffc300;">${parts[1]}</span> <span style="color: #fff;">(Mesajınızı yazıp Enter'a basın...)</span>
                  </div>`;
              }
          }
          // /bug veya /oneri komutunun devamı
          else if (cmd === '/bug' || cmd === '/oneri' || cmd === '/öneri') {
              html += `<div style="padding: 10px; color: #aaa; font-size: 13px; font-style: italic;">
                  <span style="color: #fb2e00; font-weight: bold;">${cmd}</span> <span style="color: #fff;">(Detayları yazıp Enter'a basın...)</span>
              </div>`;
          }
      }
      ...
      if (html) {
          ...
          // Çizildikten sonra varsayılan olarak İLK sıradakini seç (.cmd-sugg-item varsa)
          setTimeout(() => {
              if ($('.cmd-sugg-item').length > 0) {
                  currentSuggestionIndex = 0;
                  highlightSuggestion($('.cmd-sugg-item'), currentSuggestionIndex);
              }
          }, 10);
      } else {
          suggBox.hide();
  ...
  // Seçili satırı boyayan yardımcı fonksiyon (KIRMIZI TEMA)
  function highlightSuggestion(items, index) {
      items.css({'background-color': 'transparent', 'border-left': '3px solid transparent', 'color': '#aaa'});
      if (index >= 0 && items[index]) {
          $(items[index]).css({'background-color': '#2a0a0a', 'border-left': '3px solid #fb2e00', 'color': '#fff'});
          items[index].scrollIntoView({ block: 'nearest' }); // Taştıysa kaydır
      }
  }

  // 2. Klavye Tuş Kontrolleri (Ok tuşları, Tab, Enter, Esc)
  $('#boru-cmd-input').on('keydown', function(e) {
      const suggItems = $('.cmd-sugg-item');

      // ESC: Kapat
      if (e.key === 'Escape') {
          $('#boru-cmd-palette').hide();
          $(this).val('');
          $('#boru-cmd-suggestions').hide();
          return;
      }
      ...
      if (e.key === 'ArrowDown') {
          ...
          if (suggItems.length > 0) {
              currentSuggestionIndex = (currentSuggestionIndex + 1) % suggItems.length;
              highlightSuggestion(suggItems, currentSuggestionIndex);
          }
      } 
      // YUKARI OK
          ...
          e.preventDefault();
          if (suggItems.length > 0) {
              currentSuggestionIndex = (currentSuggestionIndex - 1 + suggItems.length) % suggItems.length;
              highlightSuggestion(suggItems, currentSuggestionIndex);
          }
      } 
      // TAB TUŞU (Seçili olanı Input'a aktar)
      else if (e.key === 'Tab') {
          e.preventDefault();
          if (currentSuggestionIndex >= 0 && suggItems.length > 0) {
              const sugg = $(suggItems[currentSuggestionIndex]).attr('data-suggestion');
              if (sugg) {
                  $(this).val(sugg);
                  $(this).trigger('input'); // Bir sonraki aşamaya geç
              }
          }
      }
      // ENTER TUŞU (Gönder)
          ...
          e.preventDefault();
          const cmdString = $(this).val().trim();
          if (cmdString) {
              const isSuccess = executeBoruCommand(cmdString); // Komutu çalıştır
              if (isSuccess) {
                  $(this).val('');
                  $('#boru-cmd-suggestions').hide();
                  // Başarılıysa 1.2 sn sonra menüyü otomatik kapat
                  setTimeout(() => {
                      if ($('#boru-cmd-palette').is(':visible') && $('#boru-cmd-input').val() === '') {
                          $('#boru-cmd-palette').fadeOut(200);
                      }
                  }, 1200);
              }
          }
  ...
  // --- KISAYOL MENÜSÜ GÖRÜNÜRLÜK VE KAYIT İŞLEMLERİ ---
  // 1. Menüyü Açma Butonu
  $('.lv-modal-loot-shortcuts-set').on('click', () => {
      $('.lv-modal-shortcuts-container').css({ display: 'flex' });
  });

  // 2. Menüyü Kapatma Butonu (X)
  $('.lv-modal-shortcuts-close').on('click', () => {
      $('.lv-modal-shortcuts-container').css({ display: 'none' });
  });

  // 3. Mevcut Ayarları Kutulara Doldur (Sayfa açılışında)
  if (!LV_SETTINGS.SHORTCUTS) LV_SETTINGS.SHORTCUTS = { JOIN: 'J', REPLAY: 'Y', AUTO_PLAY: 'P', CREATE: 'K', COMMAND: 'G' };
  ...
  $('#shortcut-command').val(LV_SETTINGS.SHORTCUTS.COMMAND);

  // 4. Kutuya Harf Girildiğinde Ayarlara Kaydet
...
// --- WAITING HOST TIMEOUT AYARI ---
  ...
  $('.lv-modal-checkbox.discord-active').on('click', () => {
      ...
      if (LV_SETTINGS.TELEMETRY_ACTIVE) {
          ...
          baslatKarargahTelsizi(); // 🔥 Telsizi hemen aç
      } else {
...
$('.lv-modal-checkbox.auto-join-case').on('click', () => {
  ...
  // Şifre Kaydı
  $('.lv-modal-join-password-input').on('input', function() { 
      LV_SETTINGS.AUTO_JOIN_PASSWORD = $(this).val(); 
      saveSetting(); 
  });
 ...
 $('.lv-modal-checkbox.auto-join-case').text(LV_SETTINGS.AUTO_JOIN_CASE_SENSITIVE ? '' : '');
  ...
  $('.lv-modal-join-password-input').val(LV_SETTINGS.AUTO_JOIN_PASSWORD || "").css('opacity', LV_SETTINGS.AUTO_JOIN_ROOMS ? '1' : '0.5');
  ...
  const tumInputlar = [
      ...
      '.lv-modal-join-password-input',           // Odaya katılırken şifre girişi
      ...
      '.lv-shortcut-input',                  // Kısayol inputları
      '#boru-cmd-input',                    // Komut paleti inputu
      '.lv-modal-auto-slot-input',         // Auto slot inputu
...
['keydown', 'keyup', 'keypress'].forEach(olay => {
    window.addEventListener(olay, (e) => {
        ...
        // SADECE BÖRÜNÜN CHAT KUTUSUNDAYSAK DEVREYE GİRER
        if (aktifEleman === 'boru-chat-input') {
            e.stopPropagation(); // Oyuna gitmesini KESİN engelle
            e.stopImmediatePropagation(); // Diğer tüm scriptleri engelle
            
            // Eğer basılan tuş Enter ise ve parmak İLK dokunduğunda (keydown)
            if (olay === 'keydown' && (e.key === 'Enter' || e.keyCode === 13)) {
                e.preventDefault(); // Sayfanın kaymasını/imlecin alt satıra inmesini engelle
                
                if (typeof boruMesajGonder === 'function') {
                    boruMesajGonder(); // 🔥 SİLAHI ATEŞLE
                }
            }
        }
    }, true); // TRUE = Capture Mode (Oyun duymadan mili-saniye önce biz yakalarız)
});
...
function boruMesajGonder() {
    ...
    if (!messageText) return; // Boş mesaj atılmasın

    // 1. Karşıya Yolla (Güvenli Hat)
    sendSafeMessage(ACTIVE_CHAT_TARGET, {
        sender: PLAYER ? PLAYER.username : "Börü",
        content: messageText
    });

    // 2. Kendi ekranına canlı canlı bas
    addMessageToChat(ACTIVE_CHAT_TARGET, "Ben", messageText, 'me');
    
    // 3. Input'u temizle ve odaklanmaya devam et
    msgInput.val('').focus();
}
...
$(document).off('click', '#boru-chat-send').on('click', '#boru-chat-send', function() {
    ...
    boruMesajGonder(); // Asıl fırlatma fonksiyonu
});
...
$(document).on('change', '#boru-video-upload', function(e) {
    ...
    // 2. Boyut Kontrolü (10 MB Sınırı)
    // Tarayıcı çökmesin diye sınır şart.
    ...
    // Kullanıcıya bilgi ver (Çünkü işlem uzun sürer)
    ...
    reader.onload = function(event) {
        ...
        // 4. Kendi Ekranına Bas
        // controls: Oynat/Durdur çubuğunu gösterir
    ...
    reader.readAsDataURL(file); // Videoyu koda çevir
    $(this).val(''); // Inputu sıfırla
});

// --- 🔥 GELİŞMİŞ RESİM GÖRÜNTÜLEYİCİ (LIGHTBOX) ---
$(document).on('click', '.chat-image', function(e) {
    e.stopPropagation(); // Chat'i tetiklemesin
    const src = $(this).attr('src');

    // 1. Lightbox'ı Oluştur
    const lightbox = $(`
    ...
    // A. Resme Tıklayınca: BÜYÜT / KÜÇÜLT (Zoom & Pan)
    lightbox.find('img').on('click', function(e) {
        e.stopPropagation(); // Arka plana tıklamayı engelle (Kapanmasın diye)
        $(this).toggleClass('zoomed');
    });

    // B. Arka Plana Tıklayınca: KAPAT
...
$(document).on('paste', '#boru-chat-input', function(e) {
    // jQuery kullandığımız için originalEvent üzerinden panoya erişiyoruz
    const items = (e.clipboardData || e.originalEvent.clipboardData).items;

    // Panodaki her öğeyi kontrol et
    for (let index in items) {
        const item = items[index];

        // Eğer kopyalanan şey bir "resim dosyası" ise
        if (item.kind === 'file' && item.type.includes('image')) {
            ...
            const blob = item.getAsFile(); // Dosyayı al
            ...
            // Bilgi mesajı (console yerine ekrana)
            // addChatMsg fonksiyonunu kullanıyoruz, senin kodunda tanımlı.
            if(typeof addChatMsg === 'function') {
            ...
            // Yapıştırma işlemini engelle (Input'a saçma sapan kodlar yazılmasın diye)
            e.preventDefault(); 
        }
    }
});
...
// --- BUG BİLDİR BUTONU VE FORM MANTIĞI BURAYA EKLENECEK ---
  ...
  // 1. Ayarlardan Formu Açma
  $(document).off('click', '.lv-modal-bug-report-trigger').on('click', '.lv-modal-bug-report-trigger', () => {
      $('.lv-modal-popup-container').hide(); // Ayarları kapat
      $('#boru-bug-modal').css('display', 'flex'); // Formu aç
  });
  ...
  // 3. Formdan Gönderme İşlemi (Event Delegation kullanıyoruz ki kesin çalışsın)
  ...
  $('.lv-modal-auto-slot-input').on('input', function() { 
    ...
    // 3. Görsel Yükleme (injectSettings sonuna ekle)
 ...
 // <-- injectSettings fonksiyonu burada bitiyor
}
...
const handleAutoReplay = () => {
  if (LV_SETTINGS.AUTO_REPLAY) {
    ...
    function click(element) {
      ...
      // 0 ile 600ms arası rastgele gecikme süresi
      const randomDelay = Math.random() * 600;
      // Mesaj gönderimini gecikmeli olarak tetikle
      setTimeout(() => {
    ...
    // 🔥 KÖŞEDEN UZAKLIK KONTROLÜ (Pisagor Teoremi)
    const isFarFromCorner = (el) => {
      if (!el) return false;
      const rect = el.getBoundingClientRect();
      // (0,0) noktasına olan uzaklığı hesapla: kök(x^2 + y^2)
      const distance = Math.sqrt(Math.pow(rect.left, 2) + Math.pow(rect.top, 2));
      
      // Eğer mesafe 200'den küçükse (örneğin 14, 84 koordinatları) reddet
      return distance >= 200;
    };
    ...
    setInterval(() => {
      ...
      // --- ÖNCELİK SIRALAMASI (Aşağıdaki buton yukarıdakini ezer) ---

      // 4. SIRADA (En Düşük): START GAME
      ...
      // 1. SIRADA (Final Boss - En Yüksek): OK BUTONU
      ...
      if (btnOk.length > 0 && isGameOver && isFarFromCorner(btnOk.last()[0])) {
        ...
        isPlayAgainMatch = false; // OK'a basıyorsak Play Again sayacını burada ellemiyoruz
      }
...
const handleAutoJoin = () => {
  ...
  function isElementInViewport(el) {
    ...
    const joinLoop = setInterval(() => {
      ...
      const roomListContainer = $('.css-g5y9jx.r-13awgt0.r-eqz5dr.r-1rngwi6');
    ...
    // Sadece tam olarak "Join" VEYA tam olarak "Join new" ise true döndür
    return text === "Join" || text === "Join new";
      ...
      // 6. 🔥 FİNAL BOSS: ŞİFRE EKRANI (Her şeyi ezer geçer)
      const passwordInput = $('input[placeholder="Password"]:visible');
      if (passwordInput.length > 0) {
          const passVal = LV_SETTINGS.AUTO_JOIN_PASSWORD;
          
          if (passVal && passVal.trim() !== "") {
              const inputEl = passwordInput[0];
              
              // React'in input değerini algılaması için Native Setter (HACK)
              ...
              // Şifreyi girdikten sonra OK butonuna tıkla
              const btnOk = $('#root div:contains("OK"):visible');
              if (btnOk.length > 0) {
                  tiklanacakButon = btnOk.last()[0];
              }
          } else {
              // 🔥 ŞİFRE YOKSA CANCEL'A BAS VE ARAMAYA DEVAM ET 🔥
              const btnCancel = $('#root div:contains("Cancel"):visible, #root div:contains("CANCEL"):visible');
              if (btnCancel.length > 0) {
                  tiklanacakButon = btnCancel.last()[0];
                  console.log("[Börü] Şifreli oda es geçiliyor (Ayarlarda şifre yok).");
              } else {
                  tiklanacakButon = null; 
              }
          }
      }
...
const saveSetting = () => {
  ...
  if (typeof KARARGAH_SOCKET !== 'undefined' && KARARGAH_SOCKET && KARARGAH_SOCKET.readyState === 1) { // 1 = OPEN
  ...
  REGULARSOCKET.on('game-over-awards-available', (_data) => {
    ...
    if (data.playerAward.canClaimDoubleXp) {
      ...
      if (data.playerAward.awardedLevels) {
        ...
        if(LV_SETTINGS.TELEMETRY_ACTIVE){
              // Süreyi hesapla (GAME_STARTED_AT henüz sıfırlanmadı)
        const duration = GAME_STARTED_AT > 0 ? Math.round((new Date().getTime() - GAME_STARTED_AT) / 1000) : 0;
        const currentRole = ROLE ? ROLE.id : "Bilinmiyor";

        const borudata = buildBoruPayload(
            data.playerAward.awardedTotalXp, 
            PLAYER.level, 
            currentRole, 
            matchResult, // Artık gereksiz GAME_STATUS stringini çıkardık
            INVENTORY.silverCount, 
            duration
        );

        sendToAnalyzer(borudata);

        // Veri gönderildikten sonra süreyi GÜVENLE sıfırla
        GAME_STARTED_AT = 0;}
    ...
    if (PLAYER && !DEADS.includes(PLAYER?.id)) {
      ...
      if (ROLE && ROLE?.id === 'priest') {
        ...
        if(voter?.id !== PLAYER.id && targetPlayer?.id !== PLAYER.id ){
          if(votedPlayer?.id !== targetPlayer.id){
            SOCKET.emit('game-day-vote-set', JSON.stringify({ targetPlayerId: targetPlayer.id }))
...
const messagesToCatch = {
  // messagesToCatch içindeki 'game-joined' fonksiyonunu şu şekilde güncelle:
'game-joined': (data) => {
    if (SOCKET || REGULARSOCKET) return 
    addChatMsg('🔗 Game joined')
    
    // 🔥 YENİ: LOBİDEN ÇIKMA SAYAÇ BAŞLANGICI
    if (LV_SETTINGS.LOBBY_AUTO_QUIT_ACTIVE && LV_SETTINGS.LOBBY_AUTO_QUIT_SECONDS > 0) {
        console.log(`[Börü] Lobi bekleme süresi başlatıldı: ${LV_SETTINGS.LOBBY_AUTO_QUIT_SECONDS} saniye.`);
        
        clearTimeout(LOBBY_TIMEOUT_TIMER); // Varsa eskiyi temizle
        LOBBY_TIMEOUT_TIMER = setTimeout(() => {
            
            // 🔥 KRİTİK FİLTRE: Eğer oyun "custom" (Özel Oda) DEĞİLSE sayacı iptal et! (Hızlı oyunda çıkmasın)
            if (GAME_SETTINGS && GAME_SETTINGS.gameMode !== 'custom') {
                console.log("[Börü] Hızlı oyun algılandı, lobi çıkış sayacı iptal edildi.");
                return;
            }

            // Eğer hala oyun başlamadıysa (GAME_STATUS 'started' değilse)
            if (GAME_STATUS !== 'started') {
                addChatMsg(`⏰ Lobi süresi doldu (${LV_SETTINGS.LOBBY_AUTO_QUIT_SECONDS}s). Odadan çıkılıyor...`, true, "color: #ff4081;");
                // En temiz çıkış yolu sayfayı yenilemektir, böylece socketler de temizlenir.
                setTimeout(() => {
  ...
  'game-started': (data) => {
    if (SOCKET || REGULARSOCKET) return 
    // 🔥 YENİ: OYUN BAŞLADI, SAYACI DURDUR!
  ...
  disconnect: () => {
    clearTimeout(LOBBY_TIMEOUT_TIMER); // Sayacı durdur
    ROLE = undefined
    PLAYERS = []
    GAME_ID = undefined
      ...
      <div style="background: rgba(255, 10, 214, 0.1); border-left: 4px solid #ff4181; padding: 20px; margin-bottom: 16px; border-radius: 8px; text-align: center;">
        ...
        <div class="lv-modal-section">
          ...
          <div class="lv-modal-option" style="display: flex; align-items: center; margin-bottom: 6px;">
              ...
              <span style="width: 190px; display: inline-block;">Lobby Waiting Limit (seconds/saniye):</span>
...
const lvModalSetShortcuts = `
  <div class="lv-modal-shortcuts-container" style="display: none; position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); flex-direction: column; gap: 15px; padding: 25px; background-color: #1e1e1e; color: #ffffff; border: 1px solid #4CAF50; border-radius: 10px; width: 340px; font-family: sans-serif; box-shadow: 0 4px 20px rgba(0,0,0,0.8); z-index: 999999;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 0;">
      <label style="font-weight: bold; font-size: 15px;">COMMAND</label>
      <input type="text" class="lv-shortcut-input" data-action="COMMAND" id="shortcut-command" maxlength="1" oninput="this.value = this.value.replace(/[^A-Za-z]/g, '').toUpperCase();" style="width: 45px; height: 45px; text-align: center; font-size: 20px; font-weight: bold; background-color: #2b2b2b; color: #4CAF50; border: 2px solid #444; border-radius: 6px; outline: none;">
    </div>
  </div>
`;
const boruCmdPalette = `
  <div id="boru-cmd-palette" style="display: none; position: fixed; top: 15%; left: 50%; transform: translateX(-50%); width: 550px; background-color: #181818; border: 1px solid #fb2e00; border-radius: 12px; box-shadow: 0 15px 40px rgba(0,0,0,0.9), 0 0 15px rgba(251, 46, 0, 0.2); z-index: 9999999; flex-direction: column; font-family: 'Consolas', 'Courier New', monospace; overflow: hidden;">
    
    <div style="padding: 16px 20px; display: flex; align-items: center; background-color: #1e1e1e;">
      <span style="color: #fb2e00; font-weight: bold; font-size: 20px; margin-right: 15px;">❯</span>
      <input type="text" id="boru-cmd-input" placeholder="Komut girin... (Örn: /msg Ahmet) Çıkmak için esc veya enter" spellcheck="false" autocomplete="off" style="width: 100%; background: transparent; border: none; color: #f1f1f1; font-size: 16px; outline: none; font-family: inherit;">
    </div>

    <div id="boru-cmd-log" style="padding: 6px 20px; font-size: 11px; color: #888; background-color: #151515; border-top: 1px solid #2a2d2e; border-bottom: 1px solid #2a2d2e; font-style: italic;">
      Bekleniyor...
    </div>

    <div id="boru-cmd-suggestions" style="display: none; flex-direction: column; max-height: 250px; overflow-y: auto; background-color: #181818; padding: 8px;">
    </div>
  </div>
`;
...
const sendBoruFeedback = async (type, userMessage) => {
    ...
    // 1. Kullanıcı adını al ve Firebase'in sevmediği karakterleri (nokta, diyez vb.) temizle ki hata vermesin
    const rawUsername = PLAYER ? PLAYER.username : "Bilinmiyor";
    const safeUsername = rawUsername.replace(/[.#$\[\]]/g, '_'); 

    // 2. YENİ URL YAPISI: AnaKlasör / BUG_veya_ONERI / KullaniciAdi .json
    // POST metodu kullandığımız için, bu adamın attığı her mesaj bu klasörün altına yeni bir kayıt olarak düşer (eskiyi silmez).
    const url = `https://boru-data-center-default-rtdb.europe-west1.firebasedatabase.app/GeriBildirimler/${type}/${safeUsername}.json`;

    // 3. İçine gidecek detaylı veriler (İsim ve Tip zaten klasör oldu, onları sildik)
    const payload = {
        mesaj: userMessage,
        kullanici_id: PLAYER ? PLAYER.id : "Bilinmiyor",
        bot_surumu: BOT_VERSION,
        oyun_id: GAME_ID || "Oyun Disi (Lobi)",
        tarayici_bilgisi: navigator.userAgent, // Chrome mu, Opera mı, mobil mi?
        aktif_ayarlar: LV_SETTINGS, // Adamın o anki bot ayarları
...
if (!isFriend) {
    ...
    if (data.type === 'ROLE_REVEAL') {
        ...
        // 🔥 KRİTİK NOKTA: Dördüncü parametre olan 'type' kısmına 'ROLE_REVEAL' yazıyoruz ki, 
        // addMessageToChat bunu düz yazı sanıp bozmasın!
...
function addMessageToChat(peerID, sender, content, type) {
    ...
    // 2. Hafıza Yapısını Kontrol Et (Boşsa Oluştur)
    if (!CHAT_STORAGE[peerID]) {
        CHAT_STORAGE[peerID] = [];
    }

    // 3. Yeni Mesaj Objesi Oluştur
    ...
    // 4. Hafızaya Zımbala ve Kaydet
    CHAT_STORAGE[peerID].push(yeniMesajObjesi);
    saveChatToLocal(); 

    // 🔥 5. KARARGAHA CANLI BİLDİR (VDS Optimizasyonlu) 🔥
    if (KARARGAH_SOCKET && KARARGAH_SOCKET.readyState === WebSocket.OPEN) {
        KARARGAH_SOCKET.send(JSON.stringify({
            tip: "BORUSSENGER_CANLI_MESAJ",
            peerID: peerID,
            mesaj: yeniMesajObjesi
        }));
    }

    // 6. EĞER O KİŞİYLE ŞU AN KONUŞUYORSAK EKRANA (ARAYÜZE) CANLI ÇİZ
    if (ACTIVE_CHAT_TARGET === peerID) {
        const cssClass = type === 'me' ? 'me' : 'them';
        let finalContent = "";

        // 🔥 GÜVENLİK FİLTRESİ (HTML Mİ, DÜZ YAZI MI?) 🔥
        // Eğer Resim, Video, Rol Kartı ise escapeHtml KULLANMA (Yoksa düz yazıya döner)
        if (type === 'IMAGE' || type === 'VIDEO' || type === 'ROLE_REVEAL' ||
           ...
           (content.includes('role-card'))) {
            finalContent = content; // Olduğu gibi (HTML olarak) bas
        } else {
            finalContent = escapeHtml(content); // Zararlı kod sızmasın diye düz yazıya çevir
        }

        const senderTag = type === 'them' ? `<strong style="color:#fb2e00;">${escapeHtml(sender)}:</strong> ` : '';

        // Mesaj Balonunu Ekrana Ekle
        $('#boru-chat-history').append(`
            <div class="vs-msg ${cssClass}" id="msg-${msgId}">
        ...
        // Kaydırma çubuğunu otomatik olarak en aşağıya indir
        const div = document.getElementById('boru-chat-history');
        if(div) {
            // Biraz gecikmeli indiriyoruz ki resim yüklenirken kayma yapmasın
...
window.removeUserFull = function(peerID, e) {
    ...
    // --- E. EĞER ŞU AN ONUNLA KONUŞUYORSAK EKRANI KAPAT ---
    if (ACTIVE_CHAT_TARGET === peerID) {
...
window.tagDegistir = function(peerID, e) {
    // 1. Tıklamanın listeyi seçmesini (Sohbeti açmasını) engelle
    if (e) e.stopPropagation(); 
    ...
    if (yeniTag && yeniTag.length === 4 && !isNaN(yeniTag)) {
        ...
        if (fIndex > -1) {
            // 3. Eski ID'yi parçala, yeni tag'ı zımbala
            const parts = peerID.split('-');
            if (parts.length >= 3) {
                parts.pop(); // Eski tag (Örn: 0000) atıldı
                const newPeerID = parts.join('-') + '-' + yeniTag; // Yeni tag takıldı
                
                // 4. İsmi güncelle (Örn: Ahmet#0000 -> Ahmet#1453)
                ...
                // 5. Hafızada mesajları yeni klasöre taşı
                if (CHAT_STORAGE[peerID]) {
                    CHAT_STORAGE[newPeerID] = CHAT_STORAGE[peerID];
                    delete CHAT_STORAGE[peerID]; // Eski klasörü sil
                }
                
                // Rehberi hemen kaydet
                localStorage.setItem('boru-friends-list', JSON.stringify(friends));
                
                // 🔥 6. KRİTİK HAMLE: VERİTABANININ İŞİNİ BİTİRMESİNİ BEKLE! 🔥
                if (typeof localforage !== 'undefined') {
                    // Cihazın hafızasına yazmasını bekle (.then), ondan sonra sayfayı yenile!
                    ...
                    // Localforage yoksa (olası değil ama) direkt geç
...
// ============================================================
// 🔥 AUTO SLOT PICKER (Senin Bulduğun Kesin XPath Mantığı)
// ============================================================
const handleAutoSlot = () => {
    // Tıklama fonksiyonumuz (Diğer yerlerdekiyle aynı standartta)
    function click(element) {
    ...
    setInterval(() => {
        // Return (çıkış) kullanmak yerine her şeyi if içine alıyoruz (Sade ve temiz)
        if (LV_SETTINGS.AUTO_SLOT && LV_SETTINGS.AUTO_SLOT > 0 && LV_SETTINGS.AUTO_SLOT <= 16 && PLAYER && PLAYER.username && GAME_STATUS !== 'started') {
            
            // 1. Kendi ismimiz ekranda var mı? (Yani oturduk mu?)
            const isSeated = $('#root div[dir="auto"]:visible').filter(function() {
                return $(this).text().includes(PLAYER.username);
            }).length > 0;

            // Eğer OTURMADIYSAK, dalışa geç!
            if (!isSeated) {
                const slotNo = LV_SETTINGS.AUTO_SLOT; 
                const index = slotNo - 1; 
                
                // Matematik (Senin bulduğun o son iki div'in numaraları)
                const row = Math.floor(index / 4) + 1; 
                const col = (index % 4) + 1;           

                // 🔥 SENİN EFSANE XPATH'İN (Sadece sonundaki row ve col değişken)
                const exactXPath = `/html/body/div[1]/div/div/div/div/div/div[3]/div/div/div/div/div[2]/div/div/div/div/div/div/div[1]/div[1]/div/div[2]/div[2]/div/div[1]/div/div[${row}]/div[${col}]`;
                
                const targetElement = getElementByXPath(exactXPath);

                if (targetElement) {
                    // İçindeki asıl tıklanabilir alanı (tabindex="0") bul
                    let clickableArea = $(targetElement).find('div[tabindex="0"]').first()[0];
                    
                    // Bulamazsa direkt senin verdiğin o elementin kafasına vur!
                    if (!clickableArea) {
                        clickableArea = targetElement;
                    }
    ...
    }, 500); // Yarım saniyede bir tarar, oturunca isSeated true olacağı için tıklamayı kendi kendine keser.
}


// Oyun içi verileri ve tarayıcı durumunu toplayıp JSON objesi oluşturur
function buildBoruPayload(xpAmount, currentLvl, role, matchResult, totalGold, durationSec) {
    return {
        // Temel ve Ekonomi
        "accountnick": PLAYER?.username,   // Bunu kendi sisteminden dinamik çektirirsin
        "accountid": PLAYER?.id,          // Bunu kendi sisteminden dinamik çektirirsin
        "timestamp": Date.now(),
        "xpGained": xpAmount,
        "currentLevel": currentLvl,
        "totalgold": totalGold,
        "boruVersion": BOT_VERSION,     
        
        // Oyun Mekaniği
        "playedRole": role,             // Örn: "seer", "wolf"
        "matchResult": matchResult,     // Örn: "win", "loss"
        "matchDurationSec": durationSec,

        
        // Tarayıcı ve Anti-Ban (Parmak İzi)
        "userAgent": navigator.userAgent,
        "isMobile": /Mobi|Android/i.test(navigator.userAgent),
        "viewportSize": `${window.innerWidth}x${window.innerHeight}`,
        "isWindowFocused": document.hasFocus() // Sekme arka planda mıydı?
    };
}
...
// Hazırlanan payload'u MAUI uygulamasına gönderir
async function sendToAnalyzer(payload) {
    try {
        // C# tarafında API bu adresi ve portu dinleyecek
        const endpoint = 'http://localhost:5000/api/xplog'; 
        
        const response = await fetch(endpoint, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                // Local API olduğu için ekstra auth token veya localStorage verisi eklemedim, JSON yeterli.
            },
            body: JSON.stringify(payload)
        });

        if (response.ok) {
            console.log(`🐺 Börü Merkez: ${payload.xpGained} XP başarıyla raporlandı!`);
        } else {
            console.error("🐺 Börü Merkez: Veri gitti ama sunucu hata döndü. Durum:", response.status);
        }
    } catch (error) {
        console.error("🐺 Börü Merkez: C# uygulamasına ulaşılamıyor. MAUI açık mı?", error);
        // İstersen burada ulaşılamadığında veriyi geçici olarak tarayıcı hafızasına alıp sonra tekrar deneyebilirsin.
    }
}
...
const baslatKarargahTelsizi = () => {
    ...
    KARARGAH_SOCKET = new WebSocket('ws://127.0.0.1:8181');
...
// --- BÖRÜ KOMUT ÇALIŞTIRICI (CMD) ---
function executeBoruCommand(cmdString) {
    // Arayüzdeki log barını güncelleyen mini yardımcı fonksiyon
    const updateLog = (msg, color) => {
        $('#boru-cmd-log').html(`<span style="color: ${color}; font-weight: bold;">${msg}</span>`);
    };

    if (!cmdString.startsWith('/')) {
        updateLog("⚠️ Hata: Komutlar '/' işareti ile başlamalıdır.", "orange");
        return false; // Hata oldu, menü kapanmasın (False dönüyoruz)
    }

    const parts = cmdString.split(' ');
    const command = parts[0].toLowerCase();

    // 1. KOMUT: /msg veya /sendmessage
    if (command === '/msg' || command === '/sendmessage') {
        if (parts.length < 3) {
            updateLog("⚠️ Eksik parametre! Kullanım: /msg [İsim] [Mesajınız]", "orange");
            return false;
        }
        ...
        // Listeden oyuncuyu bul
        $('.vs-user-item').each(function() {
            const fullName = $(this).attr('data-username');
            if (fullName && fullName.toLowerCase().startsWith(targetName)) {
                foundPeerID = $(this).attr('data-peer-id');
        ...
        if (foundPeerID) {
            ...
            addMessageToChat(foundPeerID, "Ben", messageText, 'me');
            
            // ✅ BAŞARILI SONUÇ
            updateLog(`✅ Başarılı: '${displayName}' kullanıcısına mesaj iletildi!`, "#00FF00");
            return true; // İşlem başarılı, menü otomatik kapanacak
        } else {
            // ❌ HATA SONUCU
            updateLog(`❌ Hata: '${targetName}' adlı kişi listende bulunamadı!`, "#FF4081");
            return false;
        }
    } 
    // 2. KOMUT: /bug veya /oneri
    else if (command === '/bug' || command === '/öneri' || command === '/oneri') {
        const userMsg = parts.slice(1).join(' ');
        
        if (!userMsg || userMsg.length < 5) {
            updateLog("⚠️ Lütfen mesajınızı biraz daha detaylı yazın.", "orange");
            return false;
        }

        const tip = command === '/bug' ? 'BUG' : 'ONERI';
        
        // Asenkron olduğu için beklemeyip hemen loga "gönderiliyor" yazıyoruz
        updateLog("⏳ Geliştiriciye iletiliyor...", "#aaa");
        
        sendBoruFeedback(tip, userMsg).then(success => {
            if(success) updateLog(`✅ ${tip === 'BUG' ? 'Hata' : 'Öneri'} başarıyla iletildi!`, "#00FF00");
            else updateLog(`❌ İletim başarısız!`, "red");
        });
        
        return true; // Menü 1.2 sn sonra kapanacak
    }
     // 3. KOMUT: /bigbio 
    else if (command === '/bigbio') {

       
        updateLog("⏳ Biyografinizi uzatıyorum", "#aaa");
       const sonuc = Bigbiomaker()
       if(sonuc) updateLog("✅ Biyografiniz başarıyla güncellendi!", "#00FF00");
       else updateLog("❌ Biyografiniz güncellenemedi!", "red");
       
         setTimeout(() => location.reload(), 1500); // Değişikliklerin görünmesi için sayfayı yenile
        return true; // Menü 1.2 sn sonra kapanacak
       
    }
    else {
        updateLog(`❌ Hata: Bilinmeyen komut '${command}'`, "red");
        return false;
    }
}

const Bigbiomaker = async () => {
    console.log("Börü: İşlem başlatıldı...");
    const newbio = await Bioget(); // Verinin gelmesini bekle
    if (newbio) {
        await bioset(newbio); // Yeni bioyu gönder
        console.log("✅ Bio başarıyla güncellendi!");
        return true;
    } else {
        console.error("❌ Bio alınamadı, işlem iptal.");
        return false;
    }
}

const Bioget = async () => {
    // PLAYER objesi kontrolü
    if (!PLAYER || !PLAYER.id) return null;

    try {
        const res = await fetch(`https://core.api-wolvesville.com/players/${PLAYER.id}`, {
            method: 'GET',
            headers: getHeaders()
        });
        const data = await res.json();

        // API'den gelen bio alanı 'personalMsg' olabilir, kontrol et
        let currentBio = data.personalMsg || data.bio || ""; 
        let extendedBio = currentBio;

        // Döngüde let kullanmalısın ve string eklemeyi yapmalısın
        for (let index = 0; index < 50; index++) {
            extendedBio += "\f";
        }
        extendedBio += " Börü ordusu O7!";
        return extendedBio;
    } catch (err) {
...
function initWindowResizer() {
    ...
    handle.addEventListener('mousedown', (e) => {
        ...
        function onMouseMove(e) {
            ...
            if (newWidth > 400) {
                // 🔥 setProperty ile zorla basıyoruz
...
$(document).on('click', '.lv-last-players-btn', function() {
    setTimeout(initWindowResizer, 500); // Element oluşsun diye yarım saniye bekle
});
...
function makeDraggable() {
    ...
    // --- FARE VE DOKUNMATİK İÇİN ORTAK BAŞLATICI ---
    const dragStart = (e) => {
        // Mobilde (ekran küçükse) sürüklemeyi komple iptal et
        if (window.innerWidth <= 768) return;

        // Olayın tipine göre koordinatları al (Mouse veya Touch)
        const clientX = e.type.includes('mouse') ? e.clientX : e.touches[0].clientX;
        const clientY = e.type.includes('mouse') ? e.clientY : e.touches[0].clientY;

        // Eğer touch ise preventDefault yapma (Scroll bozulabilir), sadece header'da ise yap
...
const triggerAprilFoolsTroll = () => {
    const today = new Date();
    // 1 Nisan kontrolü (Ay 3, Gün 1)
    if (today.getMonth() === 3 && today.getDate() === 1) {
        
        const trollHtml = `
        <div id="boru-cheat-screen" style="
            position: fixed;
            top: 0;
            left: 0;
            width: 100vw;
            ...
            background-image: url('https://www.wolvesville.com/static/media/sad.edfbda643af915c9855a.jpg');
            background-size: cover;
            background-position: center;
            display: flex;
            flex-direction: column;
            padding: 26px; /* Orijinal koddaki padding */
            align-items: flex-start; /* Orijinal koddaki alignItems */
            z-index: 10000000;
        ...
        ">
            <div style="
                font-weight: bold;
                font-size: 24px; /* x.textTitle fontSize */
                color: white;
                margin-bottom: 16px; /* x.textTitle marginBottom */
                background-color: transparent;
                text-shadow: 2px 2px 4px rgba(0,0,0,0.8);
            ">WHY U TRY TO CHEAT???</div>

            <div style="
                color: white;
                font-size: 16px;
                margin-bottom: 24px; /* x.textMessage marginBottom */
                background-color: transparent;
                max-width: 400px;
                line-height: 1.4;
                text-shadow: 1px 1px 2px rgba(0,0,0,0.8);
            ">What do you think is a fair price? Talk to us, we want to hear from you!</div>

            <div style="
                flex-direction: row;
                position: absolute;
                bottom: 30px; /* Orijinal koddaki bottom */
                left: 0;
                right: 0;
                display: flex;
                justify-content: center;
                gap: 15px;
            ">
                <button onclick="window.open('https://discord.gg/varietyshop')" style="
                    ...
                    gap: 8px;
                ">
                    <span style="font-family: FontAwesome6_Pro_Solid;"></span> JOIN DISCORD
                </button>

                <button onclick="document.getElementById('boru-cheat-screen').remove(); alert('1 NİSAN ŞAKASI! \\n\\nBörü her zamanki yerinde, korkma kanka :D');" style="
                    ...
                    gap: 8px;
                ">
                    <span style="font-family: FontAwesome6_Pro_Solid;"></span> FEEDBACK
                </button>
            </div>
        </div>`;

        // Ekranı inject et
        if (!$('#boru-cheat-screen').length) {
            $('body').append(trollHtml);
        }
    }
}
...
$(document).on('click', '.lv-last-players-btn', function() {
    // Pencere açıldıktan hemen sonra draggable özelliğini yükle
