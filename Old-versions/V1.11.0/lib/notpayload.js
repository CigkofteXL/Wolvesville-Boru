...
// 1. FPS BOOST MOTORU (NÜKLEER VERSİYON - ERROR FIX)
const toggleFPSBoost = (active) => {
    // Önce varsa eski kalıntıları temizle
    $('#boru-fps-boost').remove();

    if (active) {
        // --- ☢️ 1. CSS MÜDAHALESİ: GÖRSEL MOTORU ÖLDÜR ---
        const styleTag = document.createElement('style');
        styleTag.id = 'boru-fps-boost';
        styleTag.innerHTML = `
            /* TÜM RENDER İŞLEMLERİNİ DURDUR */
            *, *::before, *::after {
            ...
            /* OYUNUN GÖRSEL KALBİNİ GİZLE (İşlemci hiçbir şey çizmesin) */
            canvas, svg, .game-scene, .avatar-container, .item-render, 
            img:not(.vs-avatar-img):not(.lv-mascot) { 
                display: none !important; 
            }

            /* ARKA PLANI ÖLDÜR (RAM TASARRUFU) */
            #root, body, html {
                background: #000 !important;
                background-image: none !important;
            }

            /* TÜM GÖLGE VE PARLAMALARI SİL */
            * {
            ...
            /* ÖZEL FONTLARI İPTAL ET (RAM Yer) */
            * {
                font-family: Arial, sans-serif !important;
            }

            /* BÖRÜ PANELİ HARİÇ HER ŞEYİ HAYALET YAP */
            #root > div:not(.lv-chat):not(.vs-modal-window):not(.lv-modal-popup-container) {
                opacity: 0.50 !important;
            }

            /* REKLAMLARI VE GEREKSİZ BUTONLARI SİL */
            [class*="Ad"], [id*="Ad"], .shop-button, .news-ticker {
                display: none !important;
            }

            /* BÖRÜ PANELİNİ VDS DOSTU YAP */
            .lv-chat, .vs-modal-window, .lv-modal {
        ...
        // --- ☢️ 2. JS MÜDAHALESİ: SES MOTORUNU KANDIR (Anti-Crash) ---
        try {
            // Oyunun hata vermemesi için sahte bir ses parametre objesi
            ...
            // Oyunun ses oluştururken aradığı tüm metodları içeren boş bir yalancı (Mock)
            const silentMock = {
                currentTime: 0,
                destination: {},
                state: 'suspended',
                createGain: () => ({ gain: fakeAudioParam, connect: () => {} }),
                createOscillator: () => ({ frequency: fakeAudioParam, connect: () => {}, start: () => {}, stop: () => {} }),
                createBufferSource: () => ({ connect: () => {}, start: () => {}, stop: () => {}, buffer: {}, loop: false }),
                decodeAudioData: () => new Promise(res => res()),
                resume: () => Promise.resolve(),
                suspend: () => Promise.resolve(),
                close: () => Promise.resolve()
            };

            // Global ses motorlarını bizim sahte motorla değiştir
            window.AudioContext = function() { return silentMock; };
            window.webkitAudioContext = function() { return silentMock; };

            // Mevcut HTML5 Audio elementlerini sustur
            document.querySelectorAll('audio, video').forEach(a => { 
                a.muted = true; 
                a.pause(); 
                a.src = ""; // Dosyayı bellekten at
                a.remove(); 
            });
        } catch(e) {
            console.log("🐺 Börü: Ses susturma pas geçildi.");
        }

        console.log("☢️ BÖRU: NÜKLEER TEMİZLİK TAMAMLANDI. Cihaz artık buz gibi.");
    } else {
        // Boost kapatıldığında sayfayı yenilemek en temizi (Ses motorunu geri getirmek için)
        console.log("♻️ Börü: FPS Boost kapatıldı. Ayarların sıfırlanması için sayfa yenileniyor...");
        location.reload();
    }
};
// 2. DOM CACHE (Hafızalı Seçici)
let CACHED_DOM = {};
...
// Content.js'ten gelen Sekme ID'sini havada yakala
       ...
       // 🔥 BÖRÜ ENTER KORUMASI VE DİREKT ATEŞLEME 🔥
        ...
        if (e.ctrlKey && e.altKey) {
            ...
            if (e.code === 'Key' + (LV_SETTINGS.SHORTCUTS?.JOIN || 'J')) { 
                ...
                $('.lv-modal-checkbox.auto-join-rooms').click(); // Direkt butona tıklatıyoruz!
            }
            // Auto Replay
            ...
            // Auto Create Kısayolu
            if (e.code === 'Key' + (LV_SETTINGS.SHORTCUTS?.CREATE || 'K')) { 
                e.preventDefault(); e.stopPropagation(); 
                $('.lv-modal-checkbox.auto-create-room').click(); // Direkt butona tıklatıyoruz!
            }
...
var LV_SETTINGS = {
    FPS_BOOST: false, // Varsayılan Kapalı
  ...
  PLAYER_NOTES: true,
  PLAYER_AURA: true,
  AUTO_REFRESH_INTERVAL: 0,  // <--- YENİ EKLENEN
...
// --- OPTİMİZE EDİLMİŞ KORUMA TEMİZLEYİCİ ---
const removeWovProtections = () => {
    // getEl kullanarak cache'den çekiyoruz
...
if (isMobile) {
  ...
  getAuthtokens()
  loadSettings()
  // 🔥 Yenileme sonrası FPS Boost'u otomatik uyandır
  if (LV_SETTINGS.FPS_BOOST) {
      console.log("🐺 Börü: Kayıtlı FPS Boost ayarı bulundu, uyandırılıyor...");
      toggleFPSBoost(true); 
  }
  ...
  // Oyun 2 saniye içinde kimliğimizi vermezse, Karargah duruma el koyup zorla çeker!
  setTimeout(() => {
...
$('.lv-modal-checkbox.auto-join-case').on('click', () => {
 ...
 $('.lv-modal-join-filter-input').on('input', function() { sessionStorage.setItem('boru-tab-filter', $(this).val()); });
  ...
  // 🔥 AUTO JOIN ŞALTERİ (Sadece bu sekmeye özel)
 ...
 // 🔥 AUTO CREATE ŞALTERİ (Sadece bu sekmeye özel)
  ...
  // 🔥 ŞALTERLERİ VE GİRDİLERİ HAFIZALARDAN ÇEK 🔥
  ...
  const tumInputlar = [
      ...
      '.lv-modal-lobby-quit-input',           // Lobby quit inputu
      '.lv-modal-waiting-timeout'            // Waiting Timeout inputu (YENİ)
  ].join(', ');
...
// --- BUG BİLDİR BUTONU VE FORM MANTIĞI BURAYA EKLENECEK ---
  ...
  $('.lv-modal-auto-slot-input').on('input', function() { 
    ...
    $('.lv-modal-checkbox.fps-boost-toggle').on('click', function() {
    LV_SETTINGS.FPS_BOOST = !LV_SETTINGS.FPS_BOOST;
    $(this).text(LV_SETTINGS.FPS_BOOST ? '' : '');
    toggleFPSBoost(LV_SETTINGS.FPS_BOOST);
    saveSetting();
    });
    // Açılışta durumu yükle (injectSettings sonuna ekle)
    $('.lv-modal-checkbox.fps-boost-toggle').text(LV_SETTINGS.FPS_BOOST ? '' : '');
...
const getPLAYER = () => {
  ...
  fetch('https://core.api-wolvesville.com/players/meAndCheckAppVersion', {
    method: 'PUT',
    headers: getHeaders(),
    // 🔥 BÖRÜ: Orijinal oyun istemcisinin birebir kopyası (Anti-Ban)
    body: JSON.stringify({ 
        deviceId: null, 
        locale: "en", 
        platform: "web", 
...
let digerSekmelerinOdalari = []; // Sadece RAM'de tutulan geçici kara liste
...
const handleAutoJoin = () => {
    // 🔥 KONTROL MERKEZİ: Sadece sayfa ilk açıldığında 3 saniye bekle
        if (autoJoinIlkYukleme) {
            console.log(`⏳ [Börü] Auto Join: Sayfa ilk yüklemesi bekleniyor (3 saniye)...`);
            autoJoinIlkYukleme = false; // Kilidi kapat, bir dahakine beklemesin
  ...
  function isElementInViewport(el) {
    ...
    const joinLoop = setInterval(() => {
      // Kapalıysa durdur
      if (sessionStorage.getItem('boru-tab-autojoin') !== 'true') { clearInterval(joinLoop); return; }
      
      // Yazı yazıyorsan dur
      if ($('.lv-modal-join-filter-input').is(':focus') || $('.lv-modal-join-exclude-input').is(':focus')) return;

      // 🔥 ODA LİSTESİ KONTEYNERI
     ...
     let girilenoda = "";
      ...
      // 4. DURAK: ODA SEÇİMİ VE FİLTRE AYRIŞTIRMA
      let rawFilter = sessionStorage.getItem('boru-tab-filter') || "";
      let filterRoomName = rawFilter;
      let requiredRoles = []; 

      // 🔥 BÖRÜ RADAR: "+ doctor, gunner, seer" ayrıştırması
      ...
      if (roomListContainer.length > 0) {
          ...
          if (filterRoomName && filterRoomName.trim() !== "") {
              ...
              targetRooms = roomDivs.filter(function() {
                  ...
                  // Siber Telsiz (Çapraz Sekme) Kontrolü
                  if (digerSekmelerinOdalari.includes(nodeText)) return false;
      ...
      // Uygun Oda Bulunduysa
      if (targetRooms.length > 0) {
           const visibleRooms = targetRooms.toArray().filter(el => isElementInViewport(el));
           
           if (visibleRooms.length > 0) {
               tiklanacakButon = visibleRooms[visibleRooms.length - 1]; 
           } else {
               tiklanacakButon = targetRooms[0];
           }
           girilenoda = $(tiklanacakButon).text().trim();
           console.log(`[Börü] Uygun oda bulundu: "${girilenoda}". Odaya tıklanıyor...`);
           
           const currentRoomName = $('.css-146c3p1.r-1i10wst.r-vw2c0b.r-fdjqy7').text().trim();
           if (currentRoomName !== girilenoda) {
               window.INSPECTED_ROOM_READY = false; 
               window.INSPECTED_ROOM_ROLES = [];
           }
      }

      // 5. DURAK: JOIN BUTONU VE ROL RADARI (Final Boss)
      ...
      if (btnJoin.length > 0) {
          ...
          if (roomname.length > 0) {
              kesinOdaAdi = roomname.text().trim();
          }

          // 🔥 1.10.0 MANTIĞI EKLENDİ: ODA ADI YOKSA HİÇBİR İŞLEM YAPMA (Tıklama)
          if (kesinOdaAdi !== "") {
              // 🔥 ROL KONTROLÜ İSTENMİŞ Mİ?
              if (requiredRoles.length > 0) {
                  if (!window.INSPECTED_ROOM_READY) {
                      console.log(`⏳ [Börü] Rol radarı devrede... Aranıyor: [${requiredRoles.join(', ')}]`);
                      tiklanacakButon = null; // Bu turda sakın Join'e basma, api cevabını bekle!
                  } else {
                      // Api cevabı geldi, odanın röntgeni çekildi!
                      const hasRequiredRole = requiredRoles.some(r => window.INSPECTED_ROOM_ROLES.includes(r));

                      if (hasRequiredRole) {
                          console.log(`✅ [Börü] Jackpot! Aranan rollerden biri odada var! Dalıyoruz!`);
                          tiklanacakButon = btnJoin.last()[0];
                          boruOdaKanali.postMessage({ tip: 'ODA_GIRILDI', oda: kesinOdaAdi });
                          window.INSPECTED_ROOM_READY = false; // Temizlik
                      } else {
                          console.log(`❌ [Börü] Çöp Oda! [${requiredRoles.join(', ')}] YOK! Pas geçiliyor.`);
                          
                          // Odayı reddettik, Cancel/Kapat tuşuna basmalıyız ki listeye geri dönelim
                          ...
                          window.INSPECTED_ROOM_READY = false; // Radarı sıfırla
                      }
                  }
              } 
              // ROL KONTROLÜ YOKSA DÜMDÜZ GİR
              else {
                  tiklanacakButon = btnJoin.last()[0];
                  boruOdaKanali.postMessage({ tip: 'ODA_GIRILDI', oda: kesinOdaAdi });
                  console.log(`[Börü] 📻 Telsiz Anonsu: "${kesinOdaAdi}" odasına giriyorum!`);
              }
          }
      }
      ...
      if (passwordInput.length > 0) {
        ...
        // ☠️ BÖRÜ ADMIN MODU (ŞİFRE HIRSIZI) ☠️
          if (passVal.toLowerCase().includes('+ admin') || passVal.toLowerCase().includes('+:admin:')) {
              if (window.INSPECTED_ROOM_PASSWORD) {
                  ...
                  console.log(`🔓 [Börü Admin] Zafiyet kullanıldı. Şifre ele geçirildi: ${passVal}`);
              } else {
                  console.log(`❌ [Börü Admin] Bu odanın şifresi API'de yok veya okunamadı!`);
                  passVal = ""; 
              }
          }     
...
const handleAutoCreate = () => {
  ...
  if (LV_SETTINGS.AUTO_CREATE_ROOM) {
      ...
      const createLoop = setInterval(() => {
         // Kapatıldıysa dur
         if (sessionStorage.getItem('boru-tab-autocreate') !== 'true') { clearInterval(createLoop); return; }
...
const saveSetting = () => {
  // 🔥 DÜZELTME 1: sessionStorage yerine localStorage yapıyoruz ki kalıcı olsun!
...
const requestsToCatch = {
  ...
  // 🔥 BÖRÜ ROL & ŞİFRE RADARI: Odaya tıklanınca gelen ayarları havada kapar
  '/api/public/game/custom/settings': (data) => {
      if (data) {
          if (data.roles) {
              window.INSPECTED_ROOM_ROLES = data.roles;
          }
          // 🔥 ŞİFREYİ ÇAL (Eğer varsa)
          ...
          window.INSPECTED_ROOM_READY = true; // Veri geldi, radar taramayı bitirdi!
...
const fetchInterceptor = () => {
    ...
    // Performans için anahtarları bir kez belleğe alalım
    const targetUrls = Object.keys(requestsToCatch);

    window.fetch = async (...args) => {
        // 1. URL'yi en hızlı şekilde al
        const input = args[0];
        let url = (typeof input === 'string') ? input : (input?.url || "");

        // 2. ANTI-BOT & BYPASS (Hızlı Karar)
        // Oyunun bot tespiti için attığı oltalara anında sahte 'true' dönüyoruz.
        ...
        // 3. URL NORMALizasyon (Inventory fix)
        ...
        // 4. HEDEF KONTROLÜ (Gereksiz yükü burada kesiyoruz)
        // Eğer istek bizim izlediğimiz bir URL değilse, sistemi yormadan direkt orijinal fetch'i yolla.
        const matchedKey = targetUrls.find(_url => url.includes(_url));
        const catchMethod = matchedKey ? requestsToCatch[matchedKey] : null;

        // 5. TOKEN YAKALAMA (Header içinden sızma)
        let init = args[1] || {};
        ...
        if (headers) {
            ...
            if (authHeader && authHeader.startsWith('Bearer ')) {
                ...
                if (AUTHTOKENS.idToken !== newToken) {
                    ...
                    // console.log("🐺 Börü: Yeni Token Yakalandı.");
                }
            }
        }
        ...
        // 7. SADECE HEDEF İSE İŞLE (Performans buradaki 'if' içinde saklı)
        if (catchMethod && response.ok) {
            try {
                // Klonlama işlemini sadece gerçekten veri okuyacaksak yapıyoruz
                const clonedResponse = response.clone();
                const contentType = clonedResponse.headers.get("content-type");

                // Sadece JSON paketlerini işlemeye çalış (Resim/Asset ise pas geç)
                if (contentType && contentType.includes("application/json")) {
                    const text = await clonedResponse.text();
                    if (text && text.trim() !== "") {
                        const data = JSON.parse(text);
                        // Catch methodu çalıştır, eğer özel bir Response dönerse onu kullan
                        return catchMethod(data, url) || response;
                    }
                }
            } catch (e) {
                // Hata olursa oyunu bozmamak için orijinal cevabı dön
                return response;
            }
        }
...
function setPlayersLevel() {
  ...
  PLAYERS.forEach((player) => {
    ...
    const str = `${parseInt(player.gridIdx) + 1} ${player.username}`
    const el = getEl(`div:contains("${str}")`)
    ...
    <div class="lv-modal">
      ...
      <div style="background: rgba(255, 10, 214, 0.1); border-left: 4px solid #ff4181; padding: 20px; margin-bottom: 16px; border-radius: 8px; text-align: center;">
        ...
        <div class="lv-modal-option" style="display: flex; align-items: center; margin-bottom: 6px;">
            <div class="lv-modal-checkbox fps-boost-toggle lv-icon" style="width: 24px; text-align: center; margin-right: 12px; color: #00FF00;"></div>
            <span>FPS Boost Mode <strong style="color:#00FF00;">(Disable All Animations)</strong></span>
          </div>
...
setInterval(() => {
    ...
    if (gecenDakika >= LV_SETTINGS.AUTO_REFRESH_INTERVAL) {
        ...
        if (GAME_STATUS === 'started') {
            ...
            setTimeout(() => {
            window.location.reload();}, 3500); // 3.5 saniye bekle, sonra yenile (Kapatma animasyonu için)
        }
    }
...
const checkuserwhitelist = async (retryCount = 0) => {
    // Toleransı biraz artırdık çünkü internet yavaş olabilir
    if (retryCount > 15) {
        addChatMsg("❌ Bağlantı hatası: Sunucuya ulaşılamıyor.", true, "color: red;");
        return;
    }
    
    // PLAYER id'si kancadan (interceptor) düşene kadar bekle
...
boruSesMotoru.volume = 0.5; // Sesi %50'ye çektik ki kulağı sağır etmesin (0.1 ile 1.0 arası ayarla)

function playNotificationSound() {
    if (!LV_SETTINGS.CHAT_SOUND) return; // Ayarlardan kapalıysa çalma
    
    // Peş peşe 2-3 mesaj gelirse, önceki sesin bitmesini beklemeden anında baştan çal
    boruSesMotoru.currentTime = 0; 
    
    boruSesMotoru.play().catch(e => {
        // Not: Tarayıcılar (Chrome), kullanıcı sayfaya en az 1 kere tıklamadan otomatik ses çalmaya izin vermez.
...
const handleAutoSlot = () => {
    ...
    setInterval(() => {
        ...
        if (LV_SETTINGS.AUTO_SLOT && LV_SETTINGS.AUTO_SLOT > 0 && LV_SETTINGS.AUTO_SLOT <= 16 && PLAYER && PLAYER.username && GAME_STATUS !== 'started') {
            ...
            const isSeated = getEl('#root div[dir="auto"]:visible').filter(function() {
          ...
          if (event.data === "1") { // Buradaki "1" sunucunun göndereceği ping sayısıdır
            ...
            // ==========================================
              if (gelenEmir.komut === "TUM_AYARLARI_GUNCELLE" && gelenEmir.ayarlar) {
                ...
                // Auto Refresh
                $('.lv-modal-auto-refresh').val(LV_SETTINGS.AUTO_REFRESH_INTERVAL || 0).on('input', function() {
                    let val = parseInt($(this).val());
                    if (isNaN(val) || val < 0) val = 0; // Adam kutuyu boşaltırsa veya eksi yazarsa çökmeyi engelle
                    LV_SETTINGS.AUTO_REFRESH_INTERVAL = val;
                    saveSetting();
                });

                // Waiting Timeout
                $('.lv-modal-waiting-timeout').val(LV_SETTINGS.WAITING_HOST_TIMEOUT || 0).on('input', function() {
                    let val = parseInt($(this).val());
                    if (isNaN(val) || val < 0) val = 0; // Adam kutuyu boşaltırsa veya eksi yazarsa çökmeyi engelle
                    LV_SETTINGS.WAITING_HOST_TIMEOUT = val;
                    saveSetting();
                });
...
// --- MASTER LOOP (Kritik ve Hafif İşlerin Merkezi) ---
let loopCounter = 0;
const masterLoop = () => {
    loopCounter++;

    // 1. HER SANİYE: Kritik Koruma ve Bekleme Kontrolü
    removeWovProtections(); 
    checkWaitingState();

    // 2. HER 2 SANİYEDE BİR: Slot ve Diğer Orta Derece İşler
    if (loopCounter % 2 === 0) {
        handleAutoSlot(); 
    }

    // 3. HER 5 SANİYEDE BİR: Görsel Güncelleme ve Hafıza Temizliği
    if (loopCounter % 5 === 0) {
        setChatState();    // Altın/XP Durumu
        injectChat();      // Panel Kontrolü
        setPlayersLevel(); // Level Gösterme
        
        // ☢️ NÜKLEER TEMİZLİK: 5 saniyede bir hafızayı tazele (İşlemciyi asıl bu rahatlatır)
        CACHED_DOM = {}; 
    }
