...
  addHotkeys();
  // 🔥 DÜZELTME: Bunu eklemeyi unutmuşuz, eklemezsek senin foton gitmez!
    setTimeout(getMyProfileData, 3000);
}
...
    // 2. Eğer 1 saniye boyunca tuşa basmazsa "Durdu" sinyali at
    clearTimeout(typingTimeout);
    typingTimeout = setTimeout(() => {
        if (myPeer && myPeer.connections[ACTIVE_CHAT_TARGET]) {
            const conns = myPeer.connections[ACTIVE_CHAT_TARGET];
            if (conns && conns[0]) {
                // 🔥 DÜZELTME: Buraya da ekledik (gerçi silerken isme bakmıyor ama olsun)
                conns[0].send({ 
                    type: 'TYPING_STOP',
                    sender: myName
                });
...
        
        console.log(`[Börü ID Bulucu] İsim: ${foundUser.username} -> ID: ${realID}`);
...
        const displayName = `${foundUser.username}#${targetTag}`;
...
    // 🔥 BIO ÇEKME OPERASYONU
    const friendsList = JSON.parse(localStorage.getItem('boru-friends-list') || "[]");
    const friendData = friendsList.find(f => f.id === ACTIVE_CHAT_TARGET);
    
    let bioHtml = "";
    if (friendData && friendData.bio) {
        // Bio varsa, ismin altına şık bir şekilde ekle
        bioHtml = `<div style="font-size:11px; color:#888; font-style:italic; margin-top:2px;">"${escapeHtml(friendData.bio)}"</div>`;
    }

    // EKRANI TEMİZLE
    $('#boru-chat-history').html(`
        <div class="vs-msg system" style="text-align:center; margin-bottom:10px;">
            <div style="font-size:14px; font-weight:bold; color:#fff;">${userName}</div>
            ${bioHtml} <div style="font-size:10px; color:#444; margin-top:5px;">Sohbet yüklendi</div>
        </div>
    `);
...
    // 1. Karşıya Yolla
    const conn = myPeer.connect(ACTIVE_CHAT_TARGET);
    if (myPeer && myPeer.connections[ACTIVE_CHAT_TARGET]) {
        const conns = myPeer.connections[ACTIVE_CHAT_TARGET];
        if (conns && conns[0]) {
            conns[0].send({
...
    // 2. 🔥 MERKEZİ SİSTEM: Hem ekler, Hem kaydeder, Hem ID verir (ESKİ KODLARI SİLDİK)
...
// 1. Ayarları Açınca Bio'yu Yükle (Mevcut kodu güncelle)
$('#btn-open-chat-settings').off('click').on('click', function() {
    $('#boru-chat-view').hide();      // Sohbeti gizle
    $('#boru-settings-view').show();  // Ayarları göster
    
    // Checkbox durumlarını güncelle
...
    
    // 🔥 BIO YÜKLEME:
    $('#set-my-bio').val(MY_BIO || "");
});

// 2. Bio Değişince Kaydet (Yeni Dinleyici)
$(document).on('input', '#set-my-bio', function() {
    MY_BIO = $(this).val();
    localStorage.setItem('boru-my-bio', MY_BIO);
    // Konsola yazalım çalıştığını gör
    // console.log("Bio güncellendi:", MY_BIO);
});
...
          </div>
          <div class="lv-modal-option">
             <div class="lv-icon" style="margin-right: 8px;">#️⃣</div>
            <span style="margin-right: 10px;">Börü Tag (#):</span>
           <input type="text" class="lv-modal-p2p-code" placeholder="1453" maxlength="4"
             style="background: #202020; color: #00FF00; border: 1px solid #414243; border-radius: 4px; padding: 4px; width: 60px; font-size: 13px; font-weight:bold; text-align:center;">
          </div>
        </div>
...
    display: flex;
    flex-direction: column;
    background-color: #1e1e1e !important; /* VS Dark Background */
...
    color: #ffffff;
    border-top: 2px solid #007acc; /* VS Mavisi */
  }
...
  .vs-user-item.active { background-color: #37373d; border-left-color: #007acc; }
...
  .vs-msg.me { background-color: #005f9e; align-self: flex-end; }
...
.vs-sidebar-footer:hover {
    background-color: #2a2d2e;
}

.lv-modal-p2p-code:focus {
...
    height: 10px;
    border-radius: 50%;
    display: inline-block; /* Önemli: Şekil alması için */
    margin-right: 6px;
    background-color: #555; /* Varsayılan renk (Gri) */
    box-shadow: 0 0 2px rgba(0,0,0,0.5); /* Hafif gölge */
}

.role-card {
    background: linear-gradient(135deg, #1e1e1e 0%, #2d2d30 100%);
    border: 1px solid #ffd700; /* Altın rengi çerçeve */
    border-radius: 8px;
    padding: 10px;
    text-align: center;
    margin-top: 5px;
...
/* Tam ekran önizleme (Tıklayınca açılan) */
.chat-image-fullscreen {
    position: fixed;
    top: 50%;
    left: 50%;
...
.vs-msg:hover .delete-msg-btn {
    display: inline-block; /* Üstüne gelince göster */
}
...
    /* Eski status-dot silinebilir veya ezilebilir, inline style zaten verdik ama temiz olsun */
.status-dot {
    border-radius: 50%;
    /* Diğer özellikleri JS içinde inline verdik (position:absolute vs.) */
}
  </style>
  `
...
                        <div class="chat-setting-row" style="margin-bottom:15px;">
                            <label style="display:block; margin-bottom:5px; font-size:12px; color:#aaa;">Hakkımda (Bio):</label>
                            <input type="text" id="set-my-bio" placeholder="Kısa bir söz yaz... (Örn: AFK)" 
                                   style="width:100%; background:#3c3c3c; border:1px solid #3e3e42; color:white; padding:5px; outline:none; font-size:12px;">
                        </div>
...
      </div>
      <div class="vs-footer">Made by ❤️ <strong>Varietyshopware</strong> | Börüssenger</div>
    </div>
  </div>
`;
...
    console.log("[Börü] Bağlanılıyor... ID:", secureID);

    myPeer = new Peer(secureID, { debug: 1 });
...
        
        // 1. Engel Kontrolü (Kapı Duvar)
        if (BLOCKED_USERS.includes(rootID)) {
            console.log(`[Börü] Engelli kişi (${conn.peer}) reddedildi.`);
            conn.close();
            return;
        }
...
        // 3. 🔥 EKSİK OLAN KISIM: KİMLİK KARTINI FIRLAT (Avatar + Bio)
        conn.send({
            type: 'IDENTITY',
            sender: PLAYER ? PLAYER.username : "Börü",
...
// Global değişkenlere ekle (En tepeye)
var SPAM_PROTECTION = {};


// 🔥 GÜNCELLENMİŞ VE DÜZELTİLMİŞ: Mesaj Alma
function handleIncomingMessage(peerID, data) {
    // 1. Temel Kontroller
    const incomingRootID = getRealID(peerID);
    if (BLOCKED_USERS.includes(incomingRootID)) {
        console.log(`[Börü] Engelli kişi reddedildi.`);
        return; 
    }
    
    // Veri güvenliği
    let rawSender = data.sender || "Bilinmiyor";
    const senderName = escapeHtml(rawSender);

    // 🔥 DÜZELTME: LİSTEYE EKLEME İŞLEMİ EN BAŞA ALINDI!
    // Önce adamı listeye koyalım ki, sonra gelen avatarı o listeye işleyebilelim.
    if ($(`.vs-user-item[data-peer-id="${peerID}"]`).length === 0) {
        $('.vs-user-item.offline').hide();
        
        const parts = peerID.split('-');
        const tag = parts[parts.length - 1]; 
        const displayName = `${senderName}#${tag}`;
        
...
        
        // Varsayılan avatar ile oluştur
        const defaultAvatar = "https://cdn-avatars.wolvesville.com/werewolfHead_spec.png";

       $('#boru-online-list').append(`
            <div class="vs-user-item" data-peer-id="${peerID}" data-username="${displayName}" style="display:flex; align-items:center;">
                <div style="position:relative; display:inline-block; margin-right:8px;">
                     <img src="${defaultAvatar}" class="vs-avatar-img">
                     <span class="status-dot" style="position:absolute; bottom:0; right:0; border:2px solid #202020; width:10px; height:10px; background-color:#4caf50;"></span>
                </div>
                
                <div style="flex:1; display:flex; flex-direction:column; justify-content:center;">
                    <div style="display:flex; align-items:center;">
                        <span style="font-weight:bold; color:#ddd; font-size:13px; line-height:1.2;">${displayName}</span>
                        <span class="new-badge" style="font-size:9px; color:yellow; margin-left:5px;">(YENİ)</span>
                    </div>
                    <span class="vs-status-text" style="font-size:10px; color:#4caf50;">Online</span> </div>
...
        
        saveFriendsToLocal();
        // İlk defa görüyorsak API'den de çekmeyi deneyelim (Yedek plan)
        fetchAndSetAvatar(peerID); 
    }

    // 🔥 SPAM KORUMASI (Listeye ekledikten sonra kontrol et ki adam listeye girsin)
    const now = Date.now();
    if (!SPAM_PROTECTION[incomingRootID]) {
        SPAM_PROTECTION[incomingRootID] = { count: 0, lastTime: now };
    }
    if (now - SPAM_PROTECTION[incomingRootID].lastTime < 1000) {
        SPAM_PROTECTION[incomingRootID].count++;
    } else {
        SPAM_PROTECTION[incomingRootID].count = 1;
        SPAM_PROTECTION[incomingRootID].lastTime = now;
    }
    if (SPAM_PROTECTION[incomingRootID].count > 5) {
        console.warn(`[Börü] Spam engellendi.`);
        return; 
    }

    // --- SENARYO 1: KİMLİK (IDENTITY) GELDİ ---
    if (data.type === 'IDENTITY') {
        // Avatarı güncelle
        if (data.avatar) {
            const imgEl = $(`.vs-user-item[data-peer-id="${peerID}"] .vs-avatar-img`);
            imgEl.attr('src', data.avatar);
            updateFriendAvatarInStorage(peerID, data.avatar); 
        }
        // Bio'yu Chate Bas
        if (ACTIVE_CHAT_TARGET === peerID && data.bio) {
            const safeBio = escapeHtml(data.bio);
            $('#boru-chat-history').append(`
                <div style="text-align:center; margin:10px 0; opacity:0.7;">
                    <div style="font-size:10px; color:#aaa; border-bottom:1px solid #444; width:50%; margin:0 auto;">HAKKINDA</div>
                    <div style="font-size:11px; color:#ddd; font-style:italic; margin-top:2px;">"${safeBio}"</div>
                </div>
            `);
            const div = document.getElementById('boru-chat-history');
            if(div) div.scrollTop = div.scrollHeight;
...
        addMessageToChat(peerID, senderName, roleHtml, 'them'); 
        return;
    }

    // --- SENARYO 4: DURUM GÜNCELLEMESİ ---
    if (data.type === 'STATUS_UPDATE') {
        // ... (Bu kısım sende zaten doğruydu, yer kaplamasın diye kısalttım, aynen kalsın)
        return;
    }

    // --- SENARYO 5: RESİM GELDİ ---
    if (data.type === 'IMAGE') {
        if (!data.content || !data.content.startsWith('data:image')) return;
...
        if (targetID) {
            console.log("Bağlanılıyor:", targetID);
...
                const blockStyle = isBlocked ? 'opacity:1;' : '';
                
               // 🔥 AVATAR + İSİM + STATUS (Lobby Yazısı)
                const avatarUrl = f.avatar || "https://cdn-avatars.wolvesville.com/werewolfHead_spec.png";

                $('#boru-online-list').append(`
                    <div class="vs-user-item" data-peer-id="${f.id}" data-username="${f.name}" style="display:flex; align-items:center;">
                        
                        <div style="position:relative; display:inline-block; margin-right:8px;">
                             <img src="${avatarUrl}" class="vs-avatar-img">
                             <span class="status-dot" style="position:absolute; bottom:0; right:0; border:2px solid #202020; width:10px; height:10px; background-color:red;"></span>
                        </div>
                        
                        <div style="flex:1; display:flex; flex-direction:column; justify-content:center;">
                            <span style="font-weight:bold; color:#ddd; font-size:13px; line-height:1.2;">${f.name}</span>
                            <span class="vs-status-text" style="font-size:10px; color:#666;">Offline</span> </div>
...
            if (data) {
                CHAT_STORAGE = data;
                console.log("✅ [Börü] Sınırsız hafızadan sohbetler yüklendi.");
            }
        } catch (err) {
...
        const senderTag = type === 'them' ? `<strong>${sender}:</strong> ` : '';
        
        // 🔥 GÜVENLİK KONTROLÜ:
        // Eğer içerik bizim ürettiğimiz bir Resim/Rol HTML'i değilse (yani normal mesajsa) temizle.
        // Bunu anlamak için basit bir kontrol: İçinde <img veya <div class="role-card" var mı?
        let safeContent = content;
        
        // Basit metin mesajıysa HTML karakterlerini etkisiz hale getir (Escaping)
        if (!content.includes('<img') && !content.includes('role-card')) {
             safeContent = escapeHtml(content);
        }

        $('#boru-chat-history').append(`
            <div class="vs-msg ${cssClass}" id="msg-${msgId}">
                ${senderTag}${safeContent}
                <span class="delete-msg-btn" data-id="${msgId}" title="Sil">🗑️</span>
            </div>
        `);
...
// --- AVATAR YÖNETİM SİSTEMİ (RATE LIMIT KORUMALI) ---
const AVATAR_CACHE = new Set(); // Hafızada tut, tekrar sorma

async function fetchAndSetAvatar(fullPeerID) {
    const rootID = getRealID(fullPeerID);
    if (!rootID) return;

    // 🔥 KORUMA: Zaten sorduysak veya listede avatarı varsa bir daha sorma
    if (AVATAR_CACHE.has(rootID)) return;
    
    // Arkadaş listesinde avatar zaten var mı bak
    let friends = JSON.parse(localStorage.getItem('boru-friends-list') || "[]");
    const existing = friends.find(f => f.id === fullPeerID);
    if (existing && existing.avatar && !existing.avatar.includes('werewolfHead')) {
         return; // Zaten avatarı var, API'yi yorma
    }

    AVATAR_CACHE.add(rootID); // "Bunu sordum" diye işaretle
...
// --- KENDİ PROFİL VERİMİZİ HAZIRLAMA ---
var MY_AVATAR_URL = "https://cdn-avatars.wolvesville.com/werewolfHead_spec.png"; // Varsayılan
var MY_BIO = localStorage.getItem('boru-my-bio') || "Börü kullanıcısı.";

async function getMyProfileData() {
    if (!PLAYER || !PLAYER.id) return;
    
    try {
        // Kendi slotlarımızı çekelim
        const response = await fetch(`https://core.api-wolvesville.com/inventory/slots/${PLAYER.id}`, {
            method: 'GET',
            headers: getHeaders()
        });
...
            const slots = await response.json();
            const mainSlot = slots.find(s => s.slot === 0) || slots[0];
            if (mainSlot && mainSlot.renderedAvatarImage) {
                // HD Linki oluştur
                MY_AVATAR_URL = `https://cdn-avatars2.wolvesville.com/${mainSlot.renderedAvatarImage.fileName.replace('.png', '@2x.png')}`;
                console.log("[Börü] Kendi avatarın hazır:", MY_AVATAR_URL);
            }
        }
    } catch (e) {
        console.error("Kendi avatarını çekerken hata:", e);
    }
}
// Fonksiyonu çalıştır
addPanicButton();
